...
export class CarImageService {
  ...
  getImagesByCarId(id: string): Observable<ListResponseModel<CarImage>> {
    return this.httpClient.get<ListResponseModel<CarImage>>(
      `${this.apiControllerUrl}/getimagesbycar?id=${id}`
    );
  }

  getFileById(id: string): Observable<string> {
    return this.httpClient.get<string>(
      `${this.apiControllerUrl}/getfilebyid?id=${id}`
    );
  }

  getCarImageUrl(id: string): string {
    return `${this.apiControllerUrl}/getfilebyid?id=${id}`;
  }

  add(carId: string, file: File): Observable<ResponseModel> {
